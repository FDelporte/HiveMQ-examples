...
import be.webtechie.hivemqsender.pi4j.components.exceptions.MeasurementException;
...
public class UltrasonicDistanceSensorComponent extends Component {
     ...
     */
    public UltrasonicDistanceSensorComponent(Console console, Context pi4j) {
        this(console, pi4j, DEFAULT_PIN_TRIGGER, DEFAULT_PIN_ECHO);
    }
     ...
     */
    public UltrasonicDistanceSensorComponent(Console console, Context pi4j, int triggerAddress, int echoAddress) {
        super(console, pi4j);
        digitalInputEcho = pi4j.create(buildDigitalInputConfig(pi4j, echoAddress));
        digitalOutputTrigger = pi4j.create(buildDigitalOutputConfig(pi4j, triggerAddress));

        objectFoundHandler = new AtomicReference<>();
        objectDisappearedHandler = new AtomicReference<>();
    ...
    protected synchronized void stopPoller() {
        if (poller != null && objectDisappearedHandler.get() == null && objectFoundHandler.get() == null) {
            poller.cancel(true);
            poller = null;
        }
    }
    ...
    public void onObjectFound(SimpleEventHandler handler) {
        objectFoundHandler.set(handler);
    ...
    public void onObjectDisappeared(SimpleEventHandler handler) {
        ...
        objectDisappearedHandler.set(handler);
    ...
    private final class Poller implements Runnable {
        ...
        public void run() {
            ...
            var newState = result <= maxRange && result >= minRange;
