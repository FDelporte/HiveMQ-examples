...
import be.webtechie.hivemqsender.pi4j.components.events.DigitalEventProvider;
import be.webtechie.hivemqsender.pi4j.components.events.FlappingEventProvider;
...
public class TiltSensorComponent extends Component implements DigitalEventProvider<TiltSensorComponent.TiltState> {
     ...
     */
    public TiltSensorComponent(Console console, Context pi4j) {
        this(console, pi4j, DEFAULT_PIN, DEFAULT_DEBOUNCE);
    }
     ...
     */
    public TiltSensorComponent(Console console, Context pi4j, int address, long debounce) {
        super(console, pi4j);
        digitalInput = pi4j.create(buildDigitalInputConfig(pi4j, address, debounce));
        shakeEventProvider = new FlappingEventProvider<>(TiltState.LEFT, TiltState.RIGHT);
        addListener(shakeEventProvider);
        addListener(this::dispatchSimpleEvents);
    }
    ...
    public void onShake(int threshold, SimpleEventHandler handler) {
        shakeEventProvider.setOptions(threshold, handler);
    }
